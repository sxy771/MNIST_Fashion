...
class BackPropagation:
    ...
    def forward(self, x):
        ...
        self.a[self.L - 1] = sigmoid(self.z[self.L - 1])
    ...
    def backward(self,x, y):
        ...
        # dL/dzi = d/dzi[–∑k={1,c} y*klog(a^k)] 
        #        = –∑k={1,c} y^k * d(log(ak))/dzi
        #        = –∑k={1,c} y^k * d(log(ak))/dak * dak/dzi 
        #        = –∑k=1cykak.dakdzi
        #        = –[yi/ai * dai/dzi + ∑k={1,≠i,c} yk/ak * dak/dzi]
        #        = –yi/ai * ai(1−ai) – ∑k={1,≠i,c} yk/ak * (akai)
        #        = –yi+yi/ai + ∑k={1,c,≠i} yk*ai
        #        = ai(yi+∑k={1,c,≠i}yk)–yi
        #        = ai+∑k={1,c} yk−yi
        #        = ai.1–yi , since ∑k=1cyk=1
        #        = ai–yi
        # dL/dZ = A–Y
        self.nabla_C_out = self.a[self.L-1] - y
        # dL/dW^L = dL/dZ^L * dZ^L/dW^L
        #         = (A^L−Y)d/dW^L(A^L−1W^L+b^L)
        #         = (A^L−Y)A^L−1
        self.delta[self.L-1]  = self.nabla_C_out * self.phi_d(self.z[self.L-1])
        self.dw[self.L-1] = np.outer(self.delta[self.L-1], self.a[self.L - 2])
        self.db[self.L-1] = self.delta[self.L-1]# For all other layers except the layer L as hidden layers we can defines as:
        # d^L/dW^L−1 = dL/dZ^L * dZ^L/dA^L−1 * dA^L−1/dZ^L−1 * dZ^L−1/dW^L−1
        #            = (A^L−Y) * d/dA^L−1(AL−1WL+bL) * d/dZ^L−1(r(Z^L−1)) * d/dW^L−1(A^L−2*W^L−1+b^L−1) (r = relu)
        #            = (A^L−Y)W^Lr′(Z^L−1)A^L−2  (r = relu) 
        ...
        for t in range(epochs):
            ...
            for k in range(num_batches):
                ...
                for l in range(1,self.L):
                    ...
                    # w = w - learning late/batch_size * deltat(w,b)
...
def main():
    bp = BackPropagation()
    bp.sgd()
    print(np.sum(bp.forward(bp.trainX[1])))
